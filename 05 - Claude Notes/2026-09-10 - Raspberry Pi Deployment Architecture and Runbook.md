...
| ~~Never tested with a real pulse~~ | **Resolved by user observation.** The owner confirmed on 2026-09-10 that the system was validated with a real finger and behaves correctly. Note the distinction: the quantitative figures in §7 (99.6 Hz, 1.2 ms bridge latency, 27.7–42.1 ms inference) were all measured with **no** skin contact. Contact-driven behaviour was separately corroborated in the logs — around 19:26–19:28 PST the gate opened on genuine contact, captured 26 windows at BPM 74–97, then closed cleanly when contact ended, which is the gate working in both directions. A quantitative re-measurement under sustained real pulsatile load has still not been taken, and should be before any figure is quoted in the thesis. |
